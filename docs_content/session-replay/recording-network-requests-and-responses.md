...
You can enable recording of network requests and responses by setting `networkRecording.recordHeadersAndBody` (see [NetworkRecordingOptions](/api/client/h-init/network-recording-options)) to `true` when initializing Highlight.
...
See [NetworkRecordingOptions](/api/client/h-init/network-recording-options) for more information on how to configure network recording.
