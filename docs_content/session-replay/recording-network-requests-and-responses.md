...
You can enable recording of network requests and responses by setting `networkRecording.recordHeadersAndBody` (see [NetworkRecordingOptions](/api/h-init)) to `true` when initializing Highlight.
...
See [NetworkRecordingOptions](/api/h-init) for more information on how to configure network recording.
